...
public class ItemServiceImpl implements ItemService {
   ...
   @Override
    public ItemModel modifyItem(ItemModel itemModel) throws BusinessException {
        if (itemModel == null) {
            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR);
        }
        if (StringUtils.isEmpty(itemModel.getTitle())
    ...
    @Override
    public List<ItemModel> find(String title) {
        if (title == null || title.equals("")) {
            return listItem();
        }
        List<ItemDO> itemDOList = itemDOMapper.find(title);
        List<ItemModel> itemModelList = itemDOList.stream().map(itemDO -> {
            ItemStockDO itemStockDO = itemStockDOMapper.selectByItemId(itemDO.getId());
            ItemModel itemModel = this.convertFromDataObject(itemDO, itemStockDO);
            return itemModel;
    ...
    @Override
    public int delete(List<ItemModel> itemModelList) {
        int res = 0;
        for (ItemModel itemModel : itemModelList) {
            itemDOMapper.deleteByPrimaryKey(itemModel.getId());
            res++;
        }
        return res;
    }
